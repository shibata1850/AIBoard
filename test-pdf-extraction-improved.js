...
async function testPdfExtraction() {
  try {
    ...
    const hasFinancial = hasFinancialContent(extractedText);
    console.log(`財務コンテンツの検出: ${hasFinancial ? '成功 ✓' : '失敗 ✗'}`);
    
    if (extractedText.length > 0) {
      console.log('テキスト抽出テスト成功 ✓');
      return true;
    } else {
      console.error('テキスト抽出テスト失敗: 抽出されたテキストが空です');
      return false;
    }
  } catch (error) {
