...
async function testFinalHTMLInfographic() {
  console.log('1. Testing HTML infographic generation with extracted data...');
  ...
  try {
    ...
    if (fs.existsSync('./temp_financial_data.json')) {
      console.log('📊 Using extracted financial data from Python...');
      testData = JSON.parse(fs.readFileSync('./temp_financial_data.json', 'utf8'));
    } else {
      console.log('📊 Using fallback test data (Python extraction not available)...');
      testData = {
        ...
        analysis: 'Professional financial analysis focusing on affiliated hospital losses',
