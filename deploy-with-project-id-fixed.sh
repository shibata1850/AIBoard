...
echo "=== AIBoard Vercelデプロイ修正スクリプト (プロジェクトID指定) ==="
...
git add vercel.json server.js deploy-with-project-id-fixed.sh
...
vercel deploy --token "$VERCEL_TOKEN" --project "$VERCEL_PROJECT_ID" --yes
