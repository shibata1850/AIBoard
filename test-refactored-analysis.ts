import { analyzeDocument } from './server/api/analyze';
...
async function runQATest() {
  try {
    console.log('=== QA Test: Refactored AIBoard Analysis Function ===');
    ...
    console.log('Executing refactored multi-step analysis...');
    
    const result = await analyzeDocument(JSON.stringify(financialData));
    
    console.log('\n=== ANALYSIS RESULT ===');
    console.log(result);
    
    const outputPath = '/home/ubuntu/repos/AIBoard/refactored-qa-test-result.md';
    ...
    console.log('\n=== FINAL RESULT: FAIL ===');
    console.log('Error during test execution:', error instanceof Error ? error.message : String(error));
    throw error;
  }
}
